...
import com.git.hitzaki.auth.auth.model.po.AuthUserRole;
...
import com.git.hitzaki.auth.auth.mapper.AuthUserRoleMapper;
...
public class WxAuthServiceImpl implements AuthService {
    ...
    //拿授权码申请令牌，查询用户
    public AuthUser wxAuth(String code) {
        //拿授权码获取access_token
        ...
        //添加用户到数据库
        AuthUser xcUser = currentProxy.addWxUser(userinfo);

        return xcUser;
    }

    @Transactional
    public AuthUser addWxUser(Map userInfo_map){

        //先取出unionid
        String unionid = (String) userInfo_map.get("unionid");
        //根据unionid查询数据库
        AuthUser xcUser = userMapper.selectOne(new LambdaQueryWrapper<AuthUser>().eq(AuthUser::getWxUnionid, unionid));
        if(xcUser!=null){
            //该用户在系统存在
            return xcUser;
        }
        ...
        AuthUserRole xcUserRole = new AuthUserRole();
    ...
    */
    ...
    public AuthUserExt execute(AuthParamsDto authParamsDto) {
        ...
        AuthUser xcUser = userMapper.selectOne(new LambdaQueryWrapper<AuthUser>().eq(AuthUser::getUsername,username));
        if(xcUser==null){
